...
void update_display() {
  // display background
  WbImageRef processed_image_ref = wb_display_image_new(display, camera_width, camera_height, wb_camera_get_image(camera_L), WB_IMAGE_BGRA);
  wb_display_image_paste(display, processed_image_ref, 0, 0, false);
...
int main(int argc, char **argv) {
  ...
  for (j = 0; j < wb_robot_get_number_of_devices(); ++j) {
    ...
    if (strcmp(name, "Sick LMS 291") == 0)
      ...
      has_gps = true;
    else if (strcmp(name, "cameraL") == 0)
      has_camera = true;
  }

  // camera device
  if (has_camera) {
    camera_L = wb_robot_get_device("cameraL");
    camera_R = wb_robot_get_device("cameraR");
    wb_camera_enable(camera_L, TIME_STEP);
    wb_camera_enable(camera_R, TIME_STEP);
    camera_width = wb_camera_get_width(camera_L);
    camera_height = wb_camera_get_height(camera_L);
    camera_fov = wb_camera_get_fov(camera_L);
  }
  ...
  while (wbu_driver_step() != -1) {
    ...
    if (i % (int)(TIME_STEP / wb_robot_get_basic_time_step()) == 0) {
      ...
      if (has_camera)
        camera_image = wb_camera_get_image(camera_L);
      if (enable_collision_avoidance)
