...
vidcap = cv2.VideoCapture('6min_seg.mp4')
...
def segImageToMask(img):
    ...
    img_out[(img_h > 57) & (img_h < 63)] = SegClass.PLANT
...
while success: # and frame_count < 2000:
    if frame_count > 10800:
        if frame_count % frame_increment == 0:
            if isMask:
                cv2.imwrite("./tst/" + "%05d_gtFine_labelIds.png" % frame_count, segImageToMask(image))
            else:
                cv2.imwrite("./tst/" + "%05d_leftImg8bit.png" % frame_count, image)

    success,image = vidcap.read()
    if success == False:
        print("Final", frame_count)
